public class DesktopComputer extends Computer{
    ...
    public DesktopComputer(int size, String model, double price, int screenWidth, int screenHeight)
    {
        ...
        if (screenWidth <= 0) throw new IllegalArgumentException("Size must be greater then zero");
        else ScreenHeight = screenHeight;
        if (screenHeight <= 0) throw new IllegalArgumentException("Size must be greater then zero");
